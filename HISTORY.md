...
- Adds pipeline event `step_end`

## 0.6.0 - 2/26/21

- Pipeline and Switch raise start/exit/end events
- Removes Filter pre_process/post_process methods in lieu of events to simplify pipeline runner
- Condenses Manifest models

## 0.5.1 - 2/18/21

- Enables addition assignment operator for appending event handlers
...
- BUGFIX: Fixes check for static methods when initializing the pipeline.
...
- Moves `Step` protocol to the `pipelayer.protocol` namespace
...
- `pipelayer.Pipeline` and `pipelayer.Filter` implement the `pipelayer.Step` protocol
- Updates type hints for pre\_/post_process arguments in `pipelayer.Filter` constructor
...
- Adds support for static/module/lambda functions as well as `pipelayer.Filter` types as Pipeline Filters
